...
// Pending receipts with customer name + gate pass + vehicle
$receipts = array();
$qr = mysqli_query($con,
  "SELECT stockin.*, gatepass.*,
          supplier.supplier_name,
          SUM(stockin.asn_qty) as qtu
   FROM stockin
   ...
   WHERE stockin.branch_id = '$branch'
     AND stockin.final = '1'
     AND stockin.gatepass_id > '0'
   GROUP BY stockin.rec_dnno
   ORDER BY stockin.stockin_id DESC"
) or die(mysqli_error($con));
while ($r = mysqli_fetch_array($qr)) { $receipts[] = $r; }

$total_count = count($receipts);
$total_qty   = array_sum(array_column($receipts, 'qtu'));

// Avatar colours cycling
$avatar_classes = array('ca-1','ca-2','ca-3','ca-4');

// Helper: initials from a string
function initials($str) {
  $words = array_filter(explode(' ', trim($str)));
  $out = '';
  foreach (array_slice($words, 0, 2) as $w) $out .= strtoupper($w[0]);
  return $out;
}
...
<head>
  ...
  <title>Sovereign WMS — Receipt</title>
  ...
  <style>
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .batch-pill { display: inline-flex; align-items: center; gap: 6px; background: var(--green-bg); border: 1px solid var(--green-bd); border-radius: 20px; padding: 5px 13px; font-size: 11px; font-weight: 500; color: var(--green); }
    .batch-dot { width: 6px; height: 6px; border-radius: 50%; background: var(--green); }
    ...
    .doc-cell { display: flex; align-items: center; gap: 10px; padding: 13px 14px; }
    .doc-icon { width: 32px; height: 32px; border-radius: 7px; background: var(--bg2); border: 1px solid var(--border); display: flex; align-items: center; justify-content: center; flex-shrink: 0; transition: all .13s; }
    .doc-icon svg { width: 14px; height: 14px; color: var(--text2); }
    ...
    .doc-num { font-family: 'JetBrains Mono', monospace; font-size: 12px; font-weight: 600; color: var(--text1); }
    .doc-date { font-size: 10.5px; color: var(--text3); margin-top: 1px; }

    .cell { padding: 13px 14px; font-size: 12px; color: var(--text1); }

    .cust-cell { display: flex; align-items: center; gap: 8px; padding: 13px 14px; }
    ...
    .cust-id { font-size: 10.5px; color: var(--text3); font-family: 'JetBrains Mono', monospace; margin-top: 1px; }
    ...
    .meta-cell { padding: 13px 14px; }
    .meta-val { font-size: 12px; color: var(--text1); font-weight: 500; }
    .meta-sub { font-size: 10.5px; color: var(--text3); margin-top: 2px; }
    ...
    .progress-wrap { width: 80px; height: 4px; background: var(--bg2); border-radius: 2px; margin-top: 6px; overflow: hidden; }
    .progress-fill { height: 100%; border-radius: 2px; background: var(--orange); }

    .row-action { opacity: 0; padding: 13px 14px; transition: opacity .13s; }
    .act-btn { display: inline-flex; align-items: center; gap: 5px; padding: 6px 13px; border-radius: 6px; font-size: 11.5px; font-weight: 500; cursor: pointer; border: none; font-family: 'Inter', sans-serif; transition: all .13s; white-space: nowrap; background: var(--navy); color: #fff; }
...
<div class="topbar">
  ...
  <div class="topbar-right">
    ...
    <div class="avatar"><?php $p=explode(' ',trim($name));$i='';foreach(array_slice($p,0,2) as $x)$i.=strtoupper($x[0]);echo htmlspecialchars($i); ?></div>
  </div>
</div>
  ...
  <div class="main">
    ...
    <div class="ph">
      <div class="ph-left">
        ...
        <div class="sub">Click a document to begin the receiving process</div>
      </div>
      <?php if ($act): ?>
      <div class="batch-pill"><div class="batch-dot"></div>Active Batch: <strong><?php echo htmlspecialchars($act); ?></strong></div>
      <?php endif; ?>
    </div>
    ...
    <div class="stats-row">
      <div class="stat-card">
        ...
        <div><div class="stat-label">Pending Receipts</div><div class="stat-value"><?php echo $total_count; ?></div><div class="stat-sub">Awaiting processing</div></div>
      </div>
      <div class="stat-card">
        ...
        <div><div class="stat-label">Total ASN Qty</div><div class="stat-value"><?php echo number_format($total_qty); ?></div><div class="stat-sub">Units across <?php echo $total_count; ?> docs</div></div>
      </div>
      <div class="stat-card">
        <div class="stat-icon si-blue"><svg viewBox="0 0 18 18" fill="none"><path d="M9 3a6 6 0 1 0 0 12A6 6 0 0 0 9 3z" stroke="currentColor" stroke-width="1.3"/><path d="M9 6v3l2 2" stroke="currentColor" stroke-width="1.3" stroke-linecap="round"/></svg></div>
        <div><div class="stat-label">Unique Customers</div><div class="stat-value"><?php echo count(array_unique(array_column($receipts,'supplier_name'))); ?></div><div class="stat-sub">In pending queue</div></div>
      </div>
      <div class="stat-card">
        <div class="stat-icon si-green"><svg viewBox="0 0 18 18" fill="none"><path d="M4 9a5 5 0 1 0 10 0A5 5 0 0 0 4 9z" stroke="currentColor" stroke-width="1.3"/><path d="M6.5 9l2 2 3-3" stroke="currentColor" stroke-width="1.3" stroke-linecap="round" stroke-linejoin="round"/></svg></div>
        <div><div class="stat-label">Active Batch</div><div class="stat-value" style="font-size:15px"><?php echo $act ? htmlspecialchars($act) : '—'; ?></div><div class="stat-sub"><?php echo $act ? 'Batch open' : 'No active batch'; ?></div></div>
      </div>
    </div>
    ...
    <div class="card">
      <div class="card-hdr">
        ...
        <div class="toolbar">
          ...
          <div class="search-wrap">
            ...
            <input type="text" id="si" placeholder="Search doc, customer, vehicle…" oninput="filterRows(this.value)">
          </div>
        </div>
      </div>
          ...
          <tbody>
            ...
            <tr><td colspan="8" style="border:none">
              <div class="empty-state">
                ...
                <div class="empty-title">No pending documents</div>
                <div class="empty-sub">All receipts have been processed, or no ASNs are ready for receiving.</div>
              </div>
            </td></tr>
            <?php else: ?>
            <form method="POST" action="final_barcode_asn1.php" enctype="multipart/form-data">
              <?php foreach ($receipts as $i => $row):
                $av_class = $avatar_classes[$i % 4];
                $ini = initials($row['supplier_name']);
                $rel = relativeDate($row['rptdate']);
                $gp_no = $row['gatepass_id'];
                $cust_id = 'CUST-' . str_pad($row['supplier_id'] ?? $i+1, 3, '0', STR_PAD_LEFT);
                $carton = ($row['uom2'] > 0) ? floor($row['qtu'] / $row['uom2']) : '—';
                $dmg = intval($row['pk_damage'] ?? 0) + intval($row['unit_damage'] ?? 0);
                $rpt_fmt = $row['rptdate'] ? date('d M, H:i', strtotime($row['rptdate'])) : '—';
              ?>
              <tr>
                <td>
                  <div class="doc-cell">
                    ...
                    <div>
                      <div class="doc-num"><?php echo htmlspecialchars($row['rec_dnno']); ?></div>
                      <div class="doc-date"><?php echo $row['indate'] ? date('d M Y', strtotime($row['indate'])) : '—'; ?></div>
                    </div>
                  </div>
                </td>
                <td>
                  <div class="cust-cell">
                    <div class="cust-av <?php echo $av_class; ?>"><?php echo htmlspecialchars($ini); ?></div>
                    <div>
                      <div class="cust-name"><?php echo htmlspecialchars($row['supplier_name']); ?></div>
                      <div class="cust-id"><?php echo $cust_id; ?></div>
                    </div>
                  </div>
                </td>
                <td class="meta-cell">
                  <div class="meta-val"><?php echo htmlspecialchars($gp_no); ?></div>
                  <div class="meta-sub">Inbound</div>
                </td>
                <td class="qty-cell">
                  <div class="qty-main"><?php echo number_format($row['qtu']); ?></div>
                  <div class="qty-detail">
                    <span class="qty-tag">Cartons: <strong><?php echo $carton; ?></strong></span>
                    <span class="qty-tag">Loose: <strong><?php echo intval($row['loose_rec'] ?? 0); ?></strong></span>
                    <span class="qty-tag" <?php if($dmg>0) echo 'style="color:var(--red)"'; ?>>Dmg: <strong><?php echo $dmg; ?></strong></span>
                  </div>
                </td>
                <td class="veh-cell">
                ...
                <td class="meta-cell">
                  <div class="meta-val"><?php echo $rpt_fmt; ?></div>
                  <div class="meta-sub"><?php echo $rel; ?></div>
                </td>
                ...
                <td class="row-action">
                  <button type="submit" name="sub" value="<?php echo htmlspecialchars($row['rec_dnno']); ?>" class="act-btn">
      ...
      <div class="tbl-footer">
        <div class="tbl-footer-note"><strong><?php echo $total_count; ?> document<?php echo $total_count!=1?'s':''; ?></strong> · <?php echo number_format($total_qty); ?> total units</div>
        <div class="tbl-footer-note">Last refreshed: <?php echo date('d M Y, H:i'); ?></div>
      </div>
    </div>

    <?php include('rec_data.php'); ?>
  </div>
</div>
